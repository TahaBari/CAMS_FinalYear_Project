...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO stud_admission (student_id, cnic, full_name, gender, dob, nationality, country, city, postal_address, residential_address, qualification, photograph, degree, program, registration_no)
    VALUES ('{$_SESSION['student_id']}', '$cnic', '$full_name', '$gender', '$dob', '$nationality', '$country', '$city', '$postal_address', '$residential_address', '$qualification_intermediate', '$photograph_path', '$degree', '$program', '$res_no')";
    ...
    foreach ($education as $edu) {
        ...
        $average = ($edu['total_marks'] > 0) ? ($edu['obtained_marks'] / $edu['total_marks']) * 100 : 0;

        // Insert each education row into the education table
        $sql = "INSERT INTO education (student_id, qualification, institute_name, passing_year, grade, obtained_marks, total_marks, average)
        VALUES ('{$_SESSION["student_id"]}', '{$edu['qualification']}', '{$edu['institute_name']}', '{$edu['passing_year']}', '{$edu['grade']}', '{$edu['obtained_marks']}', '{$edu['total_marks']}', '$average')";
    ...
    echo "<script>
        alert('Form submitted successfully! Your Registration Number is: $res_no');
...
<body>
    ...
    <button class="back-button" onclick="window.location.href='stud_dashboard.php';">
        <i class="fa fa-arrow-left"></i> <!-- Back icon -->
    </button>
